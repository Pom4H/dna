...
// No hardware terms: these defects apply to service/media/software contracts alike.
...
test("REGRESSION: IDs must be unambiguous across all contract node collections", () => {
  const value = sample();
  assert.throws(() => contract({ ...value, authorities: [{ ...value.authorities[0]!, id: "service" }, value.authorities[1]!] }), /duplicate|collision/);
});
